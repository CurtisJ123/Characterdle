...
public static class EpisodeLadderRules
{
    ...
    private static readonly int[][] EasyGapPatterns = CreateGapPatterns(total: 15, min: 2, max: 6);
    private static readonly int[][] MediumGapPatterns = CreateGapPatterns(total: 10, min: 1, max: 4);
    ...
    {
        var version = difficulty is 1 or 2 ? "v4" : "v3";
        var seed = SHA256.HashData(Encoding.UTF8.GetBytes($"got:episode-ladder:{version}:{game.Id}:{difficulty}"));
    ...
    private static LadderPuzzle? Generate(LadderGameReference game, IReadOnlyList<LadderEvent> catalog, Random random, int difficulty)
    {
        ...
        var selectedEpisodes = difficulty is 1 or 2
            ? SelectBoundedEpisodes(episodes, random, difficulty == 1 ? EasyGapPatterns : MediumGapPatterns)
            : SelectLegacyEpisodes(episodes, random, difficulty);
        if (selectedEpisodes is null) return null;
    ...
    private static int[][] CreateGapPatterns(int total, int min, int max) =>
        (from first in Enumerable.Range(min, max - min + 1)
         from second in Enumerable.Range(min, max - min + 1)
         from third in Enumerable.Range(min, max - min + 1)
         let fourth = total - first - second - third
         where fourth >= min && fourth <= max
         select new[] { first, second, third, fourth }).ToArray();

    private static int[]? SelectBoundedEpisodes(Dictionary<int, LadderEvent[]> episodes, Random random, int[][] patterns)
    {
        // The first feasible pattern in a random permutation is uniform among feasible patterns.
        // Pick the anchor afterwards so patterns with more available anchors get no extra weight.
        var candidates = patterns.ToArray();
        random.Shuffle(candidates);
        var anchors = episodes.Keys.Order().ToArray();
        foreach (var gaps in candidates)
        {
            var offsets = new int[EventCount];
            for (var index = 0; index < gaps.Length; index++)
                offsets[index + 1] = offsets[index] + gaps[index] + 1;
            var validAnchors = anchors.Where(anchor => offsets.All(offset => episodes.ContainsKey(anchor + offset))).ToArray();
            if (validAnchors.Length == 0) continue;
            var start = validAnchors[random.Next(validAnchors.Length)];
            return offsets.Select(offset => start + offset).ToArray();
        }
        return null;
    }

    private static int[]? SelectLegacyEpisodes(Dictionary<int, LadderEvent[]> episodes, Random random, int difficulty)
    {
        // Preserve both selection and random-call order for the unchanged v3 difficulties.
        ...
        foreach (var anchor in anchors)
        {
            ...
            return new[] { anchor }.Concat(middleEpisodes.Take(EventCount - 2)).Append(lastEpisode).Order().ToArray();
        }
        return null;
    }
}
